...
- 让公共接口签名中出现的依赖 crate 类型从本 crate 可导入（公共接口完整性）：公共类型字段、公共函数/trait 方法签名直接使用的依赖类型必须精确重导出到使用它的域模块，消费方只依赖直接使用的库即可命名完整签名；禁止的是与公共签名无关的成套镜像转发。
- 让同一公共接口只有一条 canonical 路径；无明确跨版本责任时，在同一改动中迁移调用方并删除旧导出、代理入口和兼容别名。依赖 crate 类型的重导出不构成双路径违规：定义方仍只有一条 canonical 路径，重导出只是导入便利。
