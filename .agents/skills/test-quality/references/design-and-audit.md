...
测试设计、新增、修改或审计时读取；项目命令与授权以[AGENTS.md](../../../../AGENTS.md)为准。
...
- 进程内、跨进程或跨机器；
- 单线程、多线程、异步执行器、回调或 Dart isolate；
- 真实实现、伪实现（fake）、桩件（stub）或模拟对象（mock）；
- 是否使用文件系统、数据库、网络、子进程、原生 GUI、浏览器或外部服务；
- 环境是否由测试独占并负责建立、清理和恢复；
- 预期运行时间、门禁阶段和失败责任边界。

在能真实暴露目标缺陷的前提下，选择反馈最快、环境最稳定、失败最容易定位的层级。低层替身无法证明真实时序、协议、跨语言或宿主语义时，必须保留对应集成证据；fake 不能替代真实进程、数据库、FRB 或 provider 装配。

### 1.3 Rust 测试布局

下述为通用布局建议；本项目当前只保留 `pl-model`、`pl-core` 的公开 API 集成测试，禁止新增源码内单元测例或其他 crate 的自动化测例（以根 AGENTS.md 为准）。在允许单元测试的项目中，它验证算法、数据结构和业务逻辑的完整规则，通常放在对应生产源文件末尾的 `#[cfg(test)] mod tests` 中。

`{crate}/tests/` 放集成测试，以正常依赖方式使用被测软件包，只通过其公开 API 验证完整能力。测试辅助代码只负责准备环境，不重新实现被测功能，也不为测试扩大生产公共接口。

禁止测试通过 `#[path]` 引入生产源码，也禁止通过 `include!` 等其他源码包含方式绕过接口边界。当前项目需要验证的行为只能通过两库公开 API 的集成测试或显式人工观察证明，不为测试公开私有算法；其他项目可按各自约定选择源码内单元测试。

禁止 `unit_tests.rs` / `unit_tests/` 分离文件与 `#[cfg(test)] mod tests;` 兄弟文件形式。跨测试文件共享的 fake/fixture 收敛到 `tests/support/` 或 crate 内单一 `#[cfg(test)]` 支持模块，辅助代码不扩大生产可见性。迁移后删除无消费者的测试钩子、辅助函数和依赖；内联测试不计入生产模块行数预算。

Rust 断言优先以 `pretty_assertions::assert_eq!` 比较完整可观察对象；名称表达场景与期望。环境变量优先通过子进程配置注入；确需改当前进程环境时隔离并恢复。临时服务使用动态端口，遵守项目保留端口，且负责回收进程树。
...
新增测试前先用一句话说明：在什么输入或操作下，真实实现必须产生什么结果，哪种实现错误会破坏这个结果。无法指出真实实现及其错误后果时，不新增测试；审计已有用例时按第 3 节处置。函数可调用、对象可构造、代码可编译以及执行没有报错，均不能单独作为功能测试的理由。

1. **独立行为**：它证明哪个完整功能、算法规则或运行时不变量？边界、失败路径和历史缺陷必须归入该行为，不能单凭新增分支或问题编号成为独立用例。
2. **缺陷敏感度**：删除逻辑、反转条件、破坏返回值或恢复错误实现时，它是否会失败？
3. **独特信号**：现有测试是否已经以同等或更低成本证明同一风险？
4. **最小充分层级**：更小测试能否保持同等真实性；更大环境是否确实增加了协议、装配、时序、跨语言或平台证据？
...
- 修复错误时优先复用或增强已有功能测试，只有缺少独立行为证明时才新增；按 `AGENTS.md` 验证同一测试在错误实现上必然失败、修复后通过；
...
测试按完整行为规则组织，不按配置、参数、平台、软件包、问题编号或测例清单逐项组织。新增实例仅复用既有规则、未引入独特失败风险时不新增测试；行为改变时先检查能否复用或增强现有功能证明，只有缺少独立行为证明时才新增。将逐项用例改成遍历真实清单的参数表，并不能消除对清单的重复维护。
...
测试发现、选择器匹配、零测试拒绝、失败传播、终态判定和必要平台运行要求负责证明选中的功能确实运行。运行器的用例选择器、固定 feature profile、成功标记及协议要求的完成数量不是新增固定实例断言的理由，也不能因包含固定值而撤销。测试去重与运行矩阵去重分别判断，不能以缩减测试数量为由漏跑必需目标。
...
| 每增加一个配置、平台、软件包或测例，就增加固定该实例内容的测试 | 删除实例清单复刻，按第 2.4 节验证通用规则 | 独立兼容或历史失败行为在实际责任边界验证，不固定实例内容 |
...
- 输入、行为和判定实质相同；
- 能发现的故障种类相同；
- 更高层没有增加真实装配、契约、并发、跨语言或部署证据；
- 删除后不损失更快的定位信号、外部兼容保证或独立所有权门禁。

优先保留最低成本的充分证明。上层只保留低层无法证明的连通性、兼容性和关键旅程，不用浏览器、原生 GUI 或真实外部服务穷举局部输入组合。
...
糊来帮当前仅对 pl-model/pl-core 公开 API 设置自动集成门禁。模型层使用本地模拟供应商，core 使用脚本化模型及真实临时数据库；Studio、工具、GUI/Driver、helper 和跨语言装配不再有自动行为测例，使用人工验收核对其实际结果。模拟响应或 demo 无法证明真实供应商、原生 bridge、进程或外部服务边界。
