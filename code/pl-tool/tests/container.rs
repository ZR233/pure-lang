...
async fn workspace_file_read_has_same_json_shape_for_local_and_container_backends() {
    ...
    tokio::fs::write(root.join("a.txt"), "one\ntwo\n")
        .await
        .unwrap();
    let local = pl_tool::workspace_file::LocalWorkspaceFileBackend::new(root.clone(), false)
        .await
        .unwrap();
    let container = pl_tool::workspace_file::ContainerWorkspaceFileBackend::new(Arc::new(
    ...
    let local_output = pl_tool::workspace_file::execute_workspace_file_tool(
        &local,
        pl_tool::workspace_file::WorkspaceFileToolKind::ReadFile.name(),
        input.clone(),
    )
    .await
    .unwrap()
    .unwrap();
    let container_output = pl_tool::workspace_file::execute_workspace_file_tool(
        &container,
        pl_tool::workspace_file::WorkspaceFileToolKind::ReadFile.name(),
        input,
    )
...
async fn workspace_file_apply_patch_has_same_json_shape_for_local_and_container_backends() {
    ...
    let local_output = pl_tool::workspace_file::execute_workspace_file_tool(
        &local,
        pl_tool::workspace_file::WorkspaceFileToolKind::ApplyPatch.name(),
        input.clone(),
    )
    .await
    .unwrap()
