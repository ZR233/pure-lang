...
#[cfg(unix)]
#[tokio::test]
async fn rmcp_stdio_transport_calls_tool_and_closes_process() {
    let temp = tempfile::tempdir().unwrap();
    let pid_file = temp.path().join("server.pid");
    let connection = McpConnector::default()
...
async fn assert_real_call(connection: &pl_tool::mcp::ConnectedMcp, transport: &str) {
