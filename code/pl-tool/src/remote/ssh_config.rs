...
mod tests {
    ...
    fn profile(alias: &str, host: &str) -> SshServerProfile {
        SshServerProfile {
    ...
    fn config(dir: &tempfile::TempDir) -> SshConfigFile {
        SshConfigFile::at(dir.path().join("config"))
    }

    #[tokio::test]
    async fn parses_first_match_semantics_and_defaults() {
        let bytes = b"Host alpha\n    HostName second.example\n    HostName first.example\n    Port 2222\n    Port 22\n    User dev\n    IdentityFile \"/keys/with space\"\n\nHost beta\n";
        ...
        assert_eq!(
            ...
            vec![
                SshConfigEntry {
                    profile: SshServerProfile {
                        alias: "alpha".into(),
                        host_name: "second.example".into(),
                        port: 2222,
                        username: "dev".into(),
                        identity_file: Some("/keys/with space".into()),
                    },
                    managed: false,
                },
                SshConfigEntry {
    ...
    #[tokio::test]
    async fn skips_wildcards_multi_patterns_and_unterminated_markers() {
        let bytes = b"Host *\n    User global\n\nHost one two\n    HostName example\n\nHost ?scratch\n\n# BEGIN anywork server: dangling\nHost dangling\n    HostName example\n";
        let entries = parse_entries(bytes);
        // 未闭合标记内的 Host 仍按手写只读条目出现，但不能被 upsert 占用。
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].profile.alias, "dangling");
        assert!(!entries[0].managed);
    }

    #[tokio::test]
    async fn managed_blocks_round_trip_and_preserve_untouched_bytes() {
        let directory = tempfile::tempdir().unwrap();
        let file = config(&directory);
        let original = b"# personal servers\nHost manual\n    HostName example.test\n\n";
        std::fs::write(file.path(), original).unwrap();

        let mut entry = profile("ci", "ci.example.test");
        entry.port = 2222;
        entry.identity_file = Some("/keys/ci".into());
        file.upsert_managed(&[entry.clone()]).await.unwrap();

        let entries = file.read().await.unwrap();
        assert_eq!(entries.len(), 2);
        assert_eq!(entries[0].profile.alias, "manual");
        assert!(!entries[0].managed);
        assert_eq!(
            entries[1],
            SshConfigEntry {
        ...
        assert!(text.starts_with("# personal servers\nHost manual\n    HostName example.test\n\n"));
        assert!(text.contains("# BEGIN anywork server: ci\nHost ci\n    HostName ci.example.test\n    Port 2222\n    User dev\n    IdentityFile /keys/ci\n# END anywork server: ci\n"));

        let updated = {
            let mut changed = entry.clone();
            changed.port = 2200;
            file.upsert_managed(&[changed.clone()]).await.unwrap();
            file.read().await.unwrap()
        };
        assert_eq!(updated[1].profile.port, 2200);
        assert!(
            std::fs::read_to_string(file.path())
                .unwrap()
                .starts_with("# personal servers\nHost manual\n    HostName example.test\n\n")
        );

        file.remove_managed("ci").await.unwrap();
        assert_eq!(std::fs::read(file.path()).unwrap(), original);
    }

    #[tokio::test]
    async fn preserves_crlf_style_for_appended_blocks() {
        let directory = tempfile::tempdir().unwrap();
        let file = config(&directory);
        std::fs::write(file.path(), b"Host manual\r\n    HostName example\r\n").unwrap();
        file.upsert_managed(&[profile("ci", "ci.example")])
            .await
            .unwrap();
        let text = std::fs::read_to_string(file.path()).unwrap();
        assert!(text.contains("# BEGIN anywork server: ci\r\nHost ci\r\n"));
    ...
    #[tokio::test]
    async fn rejects_aliases_owned_by_hand_written_entries() {
        let directory = tempfile::tempdir().unwrap();
        let file = config(&directory);
        std::fs::write(file.path(), b"Host manual\n    HostName example\n").unwrap();
        let error = file
            .upsert_managed(&[profile("manual", "other")])
            .await
            .unwrap_err();
        assert!(error.to_string().contains("hand-written"));
        let error = file.remove_managed("manual").await.unwrap_err();
        assert!(error.to_string().contains("hand-written"));
        let error = file.remove_managed("missing").await.unwrap_err();
    ...
    async fn creates_missing_file_and_directory() {
        let directory = tempfile::tempdir().unwrap();
        let file = SshConfigFile::at(directory.path().join("ssh").join("config"));
        file.upsert_managed(&[profile("ci", "ci.example")])
            .await
            .unwrap();
        assert_eq!(file.read().await.unwrap().len(), 1);
    }

    #[tokio::test]
    async fn batch_upsert_replaces_and_appends_in_one_write() {
        let directory = tempfile::tempdir().unwrap();
        let file = config(&directory);
        file.upsert_managed(&[profile("one", "one.example")])
            .await
            .unwrap();
        file.upsert_managed(&[profile("one", "one-changed"), profile("two", "two.example")])
            .await
            .unwrap();
        let entries = file.read().await.unwrap();
        assert_eq!(entries[0].profile.host_name, "one-changed");
        assert_eq!(entries[1].profile.alias, "two");
        assert!(entries.iter().all(|entry| entry.managed));
    }

    #[test]
    fn alias_helpers_validate_sanitize_and_allocate() {
        assert!(validate_alias("ci-2").is_ok());
        for invalid in ["", "a b", "-lead", "a*b", "a?b", "a#b"] {
            assert!(
                validate_alias(invalid).is_err(),
                "{invalid} should be invalid"
            );
        }
        assert_eq!(sanitize_alias_base("  My CI *server* "), "My-CI-server");
        assert_eq!(sanitize_alias_base(" ??? "), DEFAULT_ALIAS_BASE);
        let taken: HashSet<String> = ["ci".to_string()].into();
        assert_eq!(allocate_alias("ci", &taken), "ci-2");
        assert_eq!(allocate_alias("fresh", &taken), "fresh");
    }
}
