...
use crate::execution::{ExecutionBackend, ExecutionRequest};
    ...
    P: GitCredentialProvider + 'static,
{
    ...
    async fn run_request(
        ...
        let outcome = GitToolOutcome::command(self.name(), output.status, stdout, stderr)?;
        if output.status == 0 {
            return Ok(outcome);
        }
        Err(PureError::Io(std::io::Error::other(GitCommandFailure {
...
#[error("git command {tool} failed with exit code {status}")]
struct GitCommandFailure {
    tool: String,
    status: i32,
    outcome: GitToolOutcome,
}

pub(super) fn observed_failure(error: &PureError) -> Option<&GitToolOutcome> {
    if let PureError::Io(error) = error {
        return error
            .get_ref()?
            .downcast_ref::<GitCommandFailure>()
...
impl GitToolOutcome {
    ...
    pub(super) fn json(tool: &str, value: serde_json::Value) -> Result<Self, PureError> {
        Ok(Self {
