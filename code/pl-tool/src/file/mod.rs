mod helpers;
mod input;
pub(crate) mod path;
pub use path::matches_pattern;

mod write;

#[doc(hidden)]
pub use input::*;

pub use write::*;

#[cfg(test)]
pub(crate) mod test_support {
    use std::path::{Path, PathBuf};
    use std::time::UNIX_EPOCH;

    pub(crate) use crate::test_support::input;
    use crate::workspace::AgentWorkspace;
    use crate::workspace::ToolWorkspace;
    use crate::workspace_file::{
        LocalWorkspaceFileBackend, ThreadWorkspaceFileTool, WorkspaceFileToolKind,
    };
    use pl_core::tool::opaque::CallContext;
    pub(crate) use pl_core::tool::opaque::Tool;
    use std::sync::Arc;
    pub(crate) fn unique_temp_dir(name: &str) -> PathBuf {
        let id = std::time::SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("pure-lang-{name}-{id}"))
    }

    pub(crate) async fn context(root: &Path) -> CallContext {
        tokio::fs::create_dir_all(root).await.unwrap();
        crate::test_support::thread_context()
    }

    pub(crate) fn tool_workspace(root: &Path) -> ToolWorkspace {
        ToolWorkspace::new(AgentWorkspace::local(root.to_path_buf()))
    }

    pub(crate) fn directory_workspace(root: &Path, paths: Option<&[&str]>) -> ToolWorkspace {
        ToolWorkspace::new(AgentWorkspace::directory(
            root.to_path_buf(),
            paths.map(|paths| paths.iter().map(|path| root.join(path)).collect()),
        ))
    }

    pub(crate) async fn file_tool(
        kind: WorkspaceFileToolKind,
        workspace: ToolWorkspace,
    ) -> ThreadWorkspaceFileTool<LocalWorkspaceFileBackend> {
        tokio::fs::create_dir_all(workspace.root()).await.unwrap();
        let backend = LocalWorkspaceFileBackend::confined(workspace.clone())
            .await
            .unwrap();
        ThreadWorkspaceFileTool::new(kind, Arc::new(backend), workspace)
    }
    pub(crate) async fn apply_patch_tool(
        root: &Path,
    ) -> ThreadWorkspaceFileTool<LocalWorkspaceFileBackend> {
        file_tool(WorkspaceFileToolKind::ApplyPatch, tool_workspace(root)).await
    }
    pub(crate) async fn write_file_tool(
        workspace: ToolWorkspace,
    ) -> crate::workspace_file::ThreadWriteFileTool<LocalWorkspaceFileBackend> {
        tokio::fs::create_dir_all(workspace.root()).await.unwrap();
        let backend = LocalWorkspaceFileBackend::confined(workspace.clone())
            .await
            .unwrap();
        crate::workspace_file::ThreadWriteFileTool::new(Arc::new(backend), workspace)
    }

    #[cfg(unix)]
    pub(crate) fn create_directory_symlink(target: &Path, link: &Path) -> std::io::Result<()> {
        std::os::unix::fs::symlink(target, link)
    }

    #[cfg(windows)]
    pub(crate) fn create_directory_symlink(target: &Path, link: &Path) -> std::io::Result<()> {
        std::os::windows::fs::symlink_dir(target, link)
    }

    #[cfg(unix)]
    pub(crate) fn remove_directory_symlink(link: &Path) -> std::io::Result<()> {
        std::fs::remove_file(link)
    }

    #[cfg(windows)]
    pub(crate) fn remove_directory_symlink(link: &Path) -> std::io::Result<()> {
        std::fs::remove_dir(link)
    }
}

#[cfg(test)]
mod apply_patch_tests {
    use pretty_assertions::assert_eq;

    use crate::workspace::ToolWorkspace;
    use crate::workspace_file::WorkspaceFileToolKind;

    use super::test_support::*;

    #[tokio::test]
    async fn apply_patch_uses_unified_input_and_json_output() {
        let root = unique_temp_dir("patch-unified-output");
        let patch = "*** Begin Patch\n*** Add File: src/lib.rs\n+pub fn ok() {}\n*** End Patch";

        let output = apply_patch_tool(&root)
            .await
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap();
        let value: serde_json::Value = serde_json::from_str(output.payload().content()).unwrap();

        assert_eq!(value["cwd"], serde_json::json!("."));
        assert_eq!(value["added"], serde_json::json!(["src/lib.rs"]));
        assert_eq!(value["changedFiles"], serde_json::json!(["src/lib.rs"]));
        assert_eq!(value["stdout"], serde_json::json!("apply_patch completed"));
        assert_eq!(
            tokio::fs::read_to_string(root.join("src/lib.rs"))
                .await
                .unwrap(),
            "pub fn ok() {}\n"
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_rejects_read_only_workspace() {
        let root = unique_temp_dir("patch-read-only");
        let tool = file_tool(
            WorkspaceFileToolKind::ApplyPatch,
            ToolWorkspace::new(crate::workspace::AgentWorkspace::confined(
                root.clone(),
                crate::workspace::WorkspaceMutability::ReadOnly,
            )),
        )
        .await;
        let patch = "*** Begin Patch\n*** Add File: denied.txt\n+denied\n*** End Patch";

        let error = tool
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap_err()
            .to_string();

        assert!(error.contains("read-only"), "{error}");
        assert!(
            !tokio::fs::try_exists(root.join("denied.txt"))
                .await
                .unwrap()
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_enforces_directory_workspace_prefixes() {
        let root = unique_temp_dir("patch-directory-prefix");
        tokio::fs::create_dir_all(root.join("allowed"))
            .await
            .unwrap();
        let tool = file_tool(
            WorkspaceFileToolKind::ApplyPatch,
            directory_workspace(&root, Some(&["allowed"])),
        )
        .await;
        let patch = "*** Begin Patch\n*** Add File: denied.txt\n+denied\n*** End Patch";

        let error = tool
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap_err()
            .to_string();

        assert!(error.contains("writablePaths"), "{error}");
        assert!(
            !tokio::fs::try_exists(root.join("denied.txt"))
                .await
                .unwrap()
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_context_mismatch_does_not_write() {
        let root = unique_temp_dir("patch-mismatch");
        tokio::fs::create_dir_all(root.join("src")).await.unwrap();
        tokio::fs::write(root.join("src/lib.rs"), "old\n")
            .await
            .unwrap();
        let patch =
            "*** Begin Patch\n*** Update File: src/lib.rs\n@@\n-missing\n+new\n*** End Patch";

        let error = apply_patch_tool(&root)
            .await
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap_err()
            .to_string();

        assert!(error.contains("Recovery: read the target file again"));
        assert_eq!(
            tokio::fs::read_to_string(root.join("src/lib.rs"))
                .await
                .unwrap(),
            "old\n"
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_accepts_and_validates_heredoc_wrappers() {
        let root = unique_temp_dir("patch-heredoc-wrapper");
        for (index, start) in ["<<EOF", "<<'EOF'", "<<\"EOF\""].into_iter().enumerate() {
            let path = format!("wrapped-{index}.txt");
            let patch =
                format!("{start}\n*** Begin Patch\n*** Add File: {path}\n+ok\n*** End Patch\nEOF");
            apply_patch_tool(&root)
                .await
                .execute(
                    input(serde_json::json!({ "input": patch })),
                    context(&root).await,
                )
                .await
                .unwrap();
            assert_eq!(
                tokio::fs::read_to_string(root.join(path)).await.unwrap(),
                "ok\n"
            );
        }

        let error = apply_patch_tool(&root).await
        .execute(
            input(serde_json::json!({
                "input": "<<\"EOF'\n*** Begin Patch\n*** Add File: bad.txt\n+nope\n*** End Patch\nEOF"
            })),
            context(&root).await,
        )
        .await
        .unwrap_err();
        assert!(error.to_string().contains("first line must be"));
        assert!(!tokio::fs::try_exists(root.join("bad.txt")).await.unwrap());
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_rejects_natural_language_instruction_with_recovery_guidance() {
        let root = unique_temp_dir("patch-natural-language");
        let error = apply_patch_tool(&root).await
        .execute(
            input(serde_json::json!({
                "input": "*** Begin Patch\nInsert after '<meta name=\"viewport\">':\n+<script></script>\n*** End Patch"
            })),
            context(&root).await,
        )
        .await
        .unwrap_err()
        .to_string();

        assert!(error.contains("natural-language edit instructions"));
        assert!(error.contains("Recovery: read the target file again"));
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_move_only_update_moves_file() {
        let root = unique_temp_dir("patch-move-only");
        tokio::fs::create_dir_all(root.join("old")).await.unwrap();
        tokio::fs::write(root.join("old/name.txt"), "same\n")
            .await
            .unwrap();
        let patch = "*** Begin Patch\n*** Update File: old/name.txt\n*** Move to: new/name.txt\n*** End Patch";

        apply_patch_tool(&root)
            .await
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap();

        assert!(
            !tokio::fs::try_exists(root.join("old/name.txt"))
                .await
                .unwrap()
        );
        assert_eq!(
            tokio::fs::read_to_string(root.join("new/name.txt"))
                .await
                .unwrap(),
            "same\n"
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_move_overwrites_existing_target() {
        let root = unique_temp_dir("patch-move-overwrite");
        tokio::fs::create_dir_all(root.join("old")).await.unwrap();
        tokio::fs::create_dir_all(root.join("new")).await.unwrap();
        tokio::fs::write(root.join("old/name.txt"), "from\n")
            .await
            .unwrap();
        tokio::fs::write(root.join("new/name.txt"), "existing\n")
            .await
            .unwrap();
        let patch = "*** Begin Patch\n*** Update File: old/name.txt\n*** Move to: new/name.txt\n@@\n-from\n+to\n*** End Patch";

        apply_patch_tool(&root)
            .await
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap();

        assert!(
            !tokio::fs::try_exists(root.join("old/name.txt"))
                .await
                .unwrap()
        );
        assert_eq!(
            tokio::fs::read_to_string(root.join("new/name.txt"))
                .await
                .unwrap(),
            "to\n"
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }

    #[tokio::test]
    async fn apply_patch_failure_reports_applied_changes() {
        let root = unique_temp_dir("patch-prefix-failure");
        tokio::fs::create_dir_all(&root).await.unwrap();
        let patch = "*** Begin Patch\n*** Add File: created.txt\n+hello\n*** Update File: missing.txt\n@@\n-old\n+new\n*** End Patch";

        let error = apply_patch_tool(&root)
            .await
            .execute(
                input(serde_json::json!({ "input": patch })),
                context(&root).await,
            )
            .await
            .unwrap_err()
            .to_string();

        assert!(error.contains("Changes applied before failure"));
        assert!(error.contains("A created.txt"));
        assert_eq!(
            tokio::fs::read_to_string(root.join("created.txt"))
                .await
                .unwrap(),
            "hello\n"
        );
        let _ = tokio::fs::remove_dir_all(root).await;
    }
}
