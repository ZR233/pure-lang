...
- `lsp_query_*`（如 `lsp_query_rust`）：当对应语言工具可用且目标语言有 active LSP 支持时，优先用于只读代码语义探索。适用场景包括定义跳转、引用查找、hover 类型/签名/文档、实现跳转、文件/workspace 符号、调用层级和 diagnostics。纯文本匹配、文件名/配置搜索、非支持语言、LSP 未激活或返回不可用错误时，回退到 `read_file`、`search_files` 或只读 `bash`/`rg`。如果只有符号名而没有文件位置，可先用 `search_files`/`read_file` 定位候选，再用对应语言的 `lsp_query_*` 做语义确认。
