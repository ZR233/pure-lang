//! `TurnEngine`：组合会话状态、模型 provider、工具注册表与单轮编译请求的核心引擎。
...
use super::tool_set::BuiltinToolInstaller;
use super::turn_loop;
use super::turn_result;

/// 生成唯一的 turn ID（毫秒时间戳 + 序列号），用于隔离每个 turn 的 trace part id。
pub(crate) fn generate_turn_id() -> String {
...
pub struct TurnEngine {
    pub(super) runtime: ModelRuntime,
    pub(super) effort: Option<ReasoningEffort>,
    pub(super) skills: Option<SkillsConfig>,
    pub(super) skill_catalog: Option<std::sync::Arc<crate::skill::FrozenSkillCatalog>>,
    pub(super) lsp_runtime: Option<pl_lsp::runtime::LspRuntimeRegistry>,
    pub(super) workspace: Option<crate::tool::AgentWorkspace>,
    pub(super) workspace_instructions: Option<String>,
    pub(super) instruction_profile: Option<crate::instruction::InstructionProfile>,
    pub(super) tool_profile: ToolProfile,
    pub(super) tool_capabilities: ToolCapabilityConfig,
    pub(super) default_turn_options: TurnOptions,
    pub(super) context_compaction: ContextCompactionConfig,
    pub(super) attachment_runtime: Option<crate::AttachmentRuntime>,
    pub(super) active_subagent: Option<SubagentContext>,
    /// 此 agent 持久拥有的工具集合；每个模型 step 都从它冻结新 plan。
    pub(super) agent_tools: AgentToolSet,
    /// 每个模型 step 冻结前由宿主更新工具集合的窗口。
    pub(super) before_model_step: Option<BeforeModelStepHook>,
    /// 仅由需要 session/working-set 的具体工具捕获。
    pub(super) tool_session_runtime: crate::tool::ToolSessionRuntime,
...
impl TurnEngine {
    ...
    pub(super) fn register_skill_tools_for_workspace(
        &mut self,
