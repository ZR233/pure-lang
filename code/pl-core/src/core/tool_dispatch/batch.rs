//! 工具调用批执行：审批、缓存、并发约束与 trace 的调度状态机。
//!
//! 单一职责说明：`execute_tool_call_batch` 是一个显式调度函数，按 solo/并行/串行
//! 约束组装工具 future 并汇合结果；记录格式化在 `display` 与 `records`，进度文案在
//! `progress_messages`。
...
use pl_protocol::InferenceOrchestrationMetrics;
...
use crate::tool::cache::{ToolCacheExecutionRequest, ToolCachePolicy};
use crate::tool::{
    ToolApprovalContext, ToolBatchPolicy, ToolBudgetTiming, ToolCallContext, ToolCallIdentity,
    ToolDirective, ToolInput, ToolRuntimeLockPolicy, WorkspaceAccess,
};
...
use super::{
    ScheduledToolExecution, ToolExecutionBatch, ToolExecutionContext, ToolExecutionError,
    ToolExecutionOutcome, ToolExecutionRecord, ToolInvocation, ToolPayload,
};

#[cfg(test)]
pub(in crate::core) async fn execute_tool_calls(
...
pub(in crate::core) async fn execute_tool_call_batch(
    tool_calls: &[pl_model::completion::ToolCall],
    budget_tracker: &mut BudgetTracker,
    recorder: &mut crate::trace::TraceRecorder,
    context: ToolExecutionContext<'_>,
...
fn namespaced_tool_trace_part_id(turn_id: &str, tool_call_id: &str) -> String {
...
pub(super) async fn notify_tool_completion(
    options: &TurnOptions,
