...
pub(crate) mod test_support {
    ...
    use crate::tool::{
        AgentWorkspace, LocalWorkspaceFileTool, ToolCallContext, ToolInput, ToolResult,
    ...
    pub(crate) async fn context(root: &Path) -> ToolCallContext {
    ...
    pub(crate) fn input(arguments: serde_json::Value) -> ToolInput {
        ToolInput { arguments }
    }

    pub(crate) trait ToolInputExecuteExt: crate::tool::StaticTool {
        fn execute_with_tool_input(
            &self,
            input: ToolInput,
    ...
    impl<T: crate::tool::StaticTool> ToolInputExecuteExt for T {}

    pub(crate) fn read_file_tool(root: &Path) -> LocalWorkspaceFileTool {
        LocalWorkspaceFileTool::new(WorkspaceFileToolKind::ReadFile, tool_workspace(root))
    }

    pub(crate) fn list_files_tool(root: &Path) -> LocalWorkspaceFileTool {
        LocalWorkspaceFileTool::new(WorkspaceFileToolKind::ListFiles, tool_workspace(root))
    }

    pub(crate) fn apply_patch_tool(root: &Path) -> LocalWorkspaceFileTool {
...
mod apply_patch_tests {
    ...
    use crate::tool::{LocalWorkspaceFileTool, ToolWorkspace, WorkspaceFileToolKind};
