...
- Chat tagged provider 的普通正文不得出现在这些标签之外；native phase provider 不要把 `<commentary>` 或 `<final>` 当作正文文本输出。不要输出 `<proposed_plan>`；需要等待确认时使用通用 `request_user_input`，工作流阶段只通过 `workflow_state` 记录。
