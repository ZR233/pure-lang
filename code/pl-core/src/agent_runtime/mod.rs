...
mod tests {
    ...
    impl ThreadRepository for TestRepository {
        ...
        async fn restore_runtime(
            &self,
        ) -> std::result::Result<Vec<RestoredAgentRuntime>, Self::Error> {
            Ok(self
                .states
                .lock()
                .unwrap()
    ...
    fn interaction_continuation(
        pending: &InteractionRequest,
    ) -> AgentInteractionContinuationRequest {
    ...
    async fn successful_rollover_replacement_and_turn_finished_share_immediate_commit() {
        ...
        let terminal_commits =
            commits
                .iter()
                .filter(|commit| {
                    commit.facts.runtime_events.iter().any(|event| {
                        matches!(event.kind, AgentRuntimeEventKind::TurnFinished { .. })
                    })
                })
                .collect::<Vec<_>>();
    ...
    async fn rejected_trace_batch_is_retried_without_a_revision_gap() {
        ...
        assert!(matches!(
            waited.last_turn.expect("budget outcome is required").outcome,
            TurnOutcome::BudgetLimited(ref budget)
                if matches!(budget.rollover(), pl_protocol::TurnRolloverOutcome::Succeeded)
        ));
        let traces = repository
            .commits()
            .into_iter()
            .flat_map(|commit| commit.facts.trace_events)
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            traces
                ...
                .collect::<Vec<_>>(),
            (0..traces.len() as u64).collect::<Vec<_>>()
        );
        assert_eq!(
            repository.state(&agent_id).session.trace_sequence,
            traces.len() as u64
        );
        runtime.shutdown().await.unwrap();
    }
        ...
        for pending in [&origin, &unrelated] {
            ...
            handle
                .submit_interaction_continuation(
                    agent_id.clone(),
                    interaction_continuation(pending),
                )
                .await
                .unwrap();
        }
