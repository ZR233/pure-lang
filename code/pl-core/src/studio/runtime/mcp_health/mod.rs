...
use anyhow::Result;
use pl_protocol::{StudioEventKind, StudioKeyValue, StudioMcpHealth, StudioMcpServer};
...
mod tests {
    ...
    use crate::config::{BuiltinMcpServerState, McpServerConfig, McpServerTransport};
    ...
    fn health_snapshot_preserves_builtin_identity_metadata() {
        ...
        let config = crate::config::PureConfig {
            builtin_mcp_servers: BTreeMap::from([(
                server_id.to_string(),
