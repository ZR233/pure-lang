...
use crate::tool::{
    AgentToolRuntime, RegisteredTool, ToolExecutionResult, strict_tool_input_schema,
};
    ...
    pub(crate) fn task_request_review_tool(
        ...
        RegisteredTool::from_typed_fallible_execution_result(
            ...
            move |_: RequestReviewInput, context| {
                ...
                async move {
                    ensure_root_task_planner(&context)?;
                    let call_id = context
                        .provider_call_id
                        .as_deref()
                    ...
                    let reviewer_snapshot = context
                        .instruction_snapshot
                        .clone()
                        .context("reviewer requires the current instruction snapshot")?
                        .with_subagent_constraint(REVIEWER_CONSTRAINT);
                    let round = coordinator
                        .store
                        .begin_task_review(&session_id, &call_id)
                        .await?;
                    drop(guard);
                    let mut run_spec = agent_runtime.run_config(
                        &context,
                    ...
                    run_spec.instruction_snapshot = Some(reviewer_snapshot);
                    let spawn = context
                        .agent_supervisor
                        .spawn_agent(
                            AgentSpawnInput {
                                task_name: format!("review_round_{}", round.round),
                                message: prompt,
...
fn ensure_root_task_planner(context: &crate::tool::ToolContext) -> Result<()> {
    ...
    {
        bail!("task_request_review requires the root Task planner");
    }
    Ok(())
}
