...
impl TaskCoordinator {
    pub(crate) fn register_conflict_tools(self: &Arc<Self>, core: &mut PureCore, session_id: &str) {
        ...
        core.register_tool(self.merge_continue_tool(session_id));
    ...
    fn merge_list_conflicts_tool(
        ...
        RegisteredTool::from_typed_fallible_execution_result(
            ...
            merge_id_schema(),
            move |input: MergeIdInput, context| {
                let coordinator = coordinator.clone();
                let session_id = session_id.clone();
                async move {
                    ensure_conflict_planner(&context)?;
    ...
    fn merge_read_conflict_tool(self: &Arc<Self>, session_id: impl Into<String>) -> RegisteredTool {
        ...
        RegisteredTool::from_typed_fallible_execution_result(
            ...
            move |input: ReadConflictInput, context| {
                let coordinator = coordinator.clone();
                let session_id = session_id.clone();
                async move {
                    ensure_conflict_planner(&context)?;
    ...
    fn merge_resolve_file_tool(self: &Arc<Self>, session_id: impl Into<String>) -> RegisteredTool {
        ...
        RegisteredTool::from_typed_fallible_execution_result(
            ...
            move |input: ResolveConflictInput, context| {
                let coordinator = coordinator.clone();
                let session_id = session_id.clone();
                async move {
                    ensure_conflict_planner(&context)?;
    ...
    fn merge_continue_tool(self: &Arc<Self>, session_id: impl Into<String>) -> RegisteredTool {
        let coordinator = self.clone();
        let session_id = session_id.into();
        RegisteredTool::from_typed_fallible_execution_result(
            "merge_continue",
            ...
            move |input: ContinueConflictInput, context| {
                let coordinator = coordinator.clone();
                let session_id = session_id.clone();
                async move {
                    ensure_conflict_planner(&context)?;
    ...
    fn merge_abort_tool(self: &Arc<Self>, session_id: impl Into<String>) -> RegisteredTool {
        ...
        RegisteredTool::from_typed_fallible_execution_result(
            ...
            move |input: AbortConflictInput, context| {
                let coordinator = coordinator.clone();
                let session_id = session_id.clone();
                async move {
                    ensure_conflict_planner(&context)?;
...
fn ensure_conflict_planner(context: &crate::tool::ToolContext) -> Result<()> {
    if context.mode != CompileMode::Task
        ...
        || !context
            .execution_profile()
            .allows_tool("merge_list_conflicts", Some(ToolEffect::ConflictWrite))
    {
        bail!("conflict tools require the root Task planner in resolvingConflict");
    }
    Ok(())
}
