...
mod tests {
    ...
    use pl_model::{
        ModelCapabilities, ModelInfo, ModelRequestProfile, TokenUsage, TruncationPolicy,
    };
    use pl_protocol::{
        AgentRuntimeDelta, AgentStatus, ContentPart, ImageSource, Message, MessageContent,
        MessageRole, RuntimeCostAmount, SkillActivation, StudioEventEnvelope, StudioEventKind,
    ...
    async fn turn_runtime_upsert_does_not_double_count_recorded_inference_usage() {
        ...
        store
            .upsert_session_runtime_for_turn(
                &session.id,
                "turn-runtime-dedupe",
                &result,
                Some(&model),
    ...
    #[tokio::test]
    async fn agent_runtime_update_persists_before_session_runtime_broadcast() {
        let store = StudioStore::open_memory().await.unwrap();
        ...
        let delta = AgentRuntimeDelta {
            inference_id: "live-runtime-1".to_string(),
            agent_id: "agent-root".to_string(),
            path: "/root".to_string(),
            parent_path: None,
            role: "root".to_string(),
        ...
        event_tx
            .send(pl_trace::AgentEvent::AgentRuntimeUpdated {
                delta: delta.clone(),
            })
            .unwrap();
        drop(event_tx);
        ...
        let event = tokio::time::timeout(Duration::from_secs(2), async {
            loop {
                ...
                if matches!(event.kind, StudioEventKind::SessionRuntimeChanged { .. }) {
                    break event;
                }
            }
        })
        .await
        .unwrap();
        let StudioEventKind::SessionRuntimeChanged { runtime } = event.kind else {
            unreachable!();
        };
        drain.await.unwrap();

        assert_eq!(runtime.usage.latest_context_tokens, 12_345);
        assert_eq!(runtime.usage.prompt_tokens, 12_345);
        assert_eq!(runtime.usage.completion_tokens, 678);
        assert_eq!(runtime.usage.total_tokens, 13_023);
        assert_eq!(runtime.usage.context_window, Some(256_000));
