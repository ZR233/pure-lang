...
use pl_core::{
    CompileMode, InteractionResolution, InteractionStatus, PlanConfirmationResolution,
...
async fn run_offline_task_flow() -> Result<()> {
    ...
    fixture
        .runtime
        .set_session_mode(&fixture.session_id, CompileMode::Simple)
        .await?;
    let session = fixture
        .store
    ...
    assert_eq!(session.mode, CompileMode::Simple.label());
