use pl_protocol::{
    ...
    ThreadSubscriptionUpdate, ThreadTextChannel, ThreadTextItem, ThreadTurnHistory, Turn,
...
async fn product_metadata_rebinds_only_the_subscription_bootstrap() {
    ...
    thread.mode = pl_protocol::ThreadModeId::task();
    ...
    assert!(matches!(
        ...
        Some(ThreadSubscriptionUpdate::Snapshot { snapshot })
            if snapshot.thread.mode == pl_protocol::ThreadModeId::task()
