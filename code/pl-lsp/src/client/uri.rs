...
mod tests {
    ...
    #[test]
    fn strips_windows_verbatim_drive_and_unc_prefixes_before_uri_encoding() {
        let drive = normalize_windows_verbatim_path(r"\\?\C:\work\pure lang\src\lib.rs");
        let unc = normalize_windows_verbatim_path(r"\\?\UNC\server\share\pure lang\src\lib.rs");

        assert_eq!(drive, r"C:\work\pure lang\src\lib.rs");
        assert_eq!(unc, r"\\server\share\pure lang\src\lib.rs");
        if cfg!(windows) {
            assert_eq!(
                path_to_file_uri(Path::new(r"\\?\C:\work\pure lang\src\lib.rs")),
                "file:///C:/work/pure%20lang/src/lib.rs"
            );
            assert_eq!(
                path_to_file_uri(Path::new(r"\\?\UNC\server\share\pure lang\src\lib.rs")),
                "file://server/share/pure%20lang/src/lib.rs"
            );
        }
    }
}
