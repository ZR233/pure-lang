const zhCN = {
  brand: {
    title: "Pure Studio",
    subtitle: "自然语言编译器",
  },

  nav: {
    settings: "设置",
    projects: "项目",
    sessions: "会话",
  },

  actions: {
    send: "发送",
    save: "保存",
    reload: "重新加载",
    approve: "批准",
    deny: "拒绝",
    back: "返回",
  },

  status: {
    starting: "启动中",
    ready: "就绪",
    running: "运行中",
    done: "完成",
    thinking: "思考中...",
    projectLoaded: "项目已加载",
    sessionLoaded: "会话已加载",
    configSaved: "配置已保存",
    configReloaded: "配置已重新加载",
    providerSettingsSaved: "Provider 设置已保存",
    toolApproved: "工具已批准",
    toolDenied: "工具已拒绝",
    bootstrapFailed: "启动失败：{{error}}",
    addProjectFailed: "添加项目失败：{{error}}",
    selectProjectFailed: "选择项目失败：{{error}}",
    newSessionFailed: "新建会话失败：{{error}}",
    selectSessionFailed: "选择会话失败：{{error}}",
    runFailed: "运行失败：{{error}}",
    configLoadFailed: "配置加载失败：{{error}}",
    configInvalid: "配置无效：{{error}}",
    providerSettingsInvalid: "Provider 设置无效：{{error}}",
    reloadFailed: "重新加载失败：{{error}}",
    approvalRequired: "需要审批：{{name}}",
    approved: "已批准：{{name}}",
    denied: "已拒绝：{{name}}",
    toolInput: "工具输入：{{name}}",
    toolCompleted: "{{name}} 已完成",
    subagentStatus: "子代理 {{status}}",
    error: "错误：{{message}}",
  },

  roles: {
    explorer: "探索者",
    planner: "计划者",
    executor: "执行者",
    reviewer: "审查者",
    explorerHint: "代码、文档和上下文探索",
    plannerHint: "默认聊天和计划生成",
    executorHint: "subagent 默认执行角色",
    reviewerHint: "代码审查和结果检查",
  },

  subagent: {
    title: "子代理",
    depth: "深度",
    parent: "父级",
    waitingToStart: "等待启动。",
    noSummaryYet: "暂无摘要。",
    queued: "排队中",
    awaitingApproval: "等待审批",
    running: "运行中",
    awaitingTool: "等待工具",
    succeeded: "已成功",
    failed: "已失败",
    denied: "已拒绝",
    toolCall: "工具调用",
    defaultWorkingDirectory: "（默认工作目录）",
    subagentLabel: "子代理 {{id}}",
  },

  conversation: {
    defaultTitle: "对话",
    addProjectHint: "添加或选择项目以开始",
    emptyTitle: "准备就绪",
    emptyDescription: "选择一个项目，然后让 Pure Studio 探索、规划或执行。",
    askPlaceholder: "向 Pure Studio 提问...",
    noSessionPlaceholder: "创建或选择一个会话",
  },

  context: {
    project: "项目",
    runtime: "运行时",
    tools: "工具",
    noProject: "暂无项目",
    chooseFolder: "选择文件夹以加载上下文。",
    noActivity: "运行后，子代理和工具活动会显示在这里。",
    sessions: "会话",
    messages: "消息",
    providers: "Provider",
  },

  approval: {
    title: "工具审批",
  },

  settings: {
    title: "设置",
    defaultConfigDraft: "默认配置草稿",
    providerRoute: "Provider 路由",
    providerRouteDesc: "配置 ~/.pure/config.toml 中的模型接入点。",
    roleRoute: "角色路由",
    roleRouteDesc: "固定四个模型角色，聊天使用计划者，subagent 默认使用执行者。",
    comingSoon: "即将推出",
    comingSoonDesc: "此设置标签页将在后续版本中实现。",
    tabs: {
      providers: "Provider",
      models: "模型",
      roles: "角色",
      security: "安全",
      general: "通用",
    },
  },

  provider: {
    defaultRoute: "默认路由",
    defaultBaseUrl: "（默认 Base URL）",
    notSelected: "未选择",
    saved: "已保存",
    notConfigured: "未配置",
    editTooltip: "编辑 provider",
    deleteTooltip: "删除 provider",
    searchPlaceholder: "搜索 provider、key 或 base URL",
    noMatchingProviders: "没有匹配的 provider",
    clearSearchHint: "清空搜索条件后可查看完整列表。",
    models: "{{count}} 个模型",
    protocolType: "协议类型",
    key: "Key",
    defaultModel: "默认模型",
    apiKey: "API Key",
    template: "模板",
    updated: "更新时间",
    platform: "{{name}} 平台",
    draft: "草稿",
    healthy: "正常",
    needsSetup: "需要配置",
    providerKey: "Provider Key",
    displayName: "显示名",
    baseUrl: "Base URL",
    addProvider: "添加 {{name}}",
  },

  model: {
    title: "模型",
    defaultModels: "默认模型",
    customModels: "自定义模型",
    customModelButton: "自定义模型",
    noCustomModels: "暂无自定义模型。",
    defaultModelDesc: "默认模型由模板提供，自定义模型追加在后面。",
    context: "上下文",
    maxOutput: "最大输出",
    autoCompact: "自动压缩",
    temperature: "温度",
    efforts: "推理强度",
    truncation: "截断",
    capabilities: "能力",
    input: "输入",
    deleteTooltip: "删除自定义模型",
    slugPlaceholder: "model-slug",
    displayNamePlaceholder: "显示名称",
    effortsPlaceholder: "high, xhigh",
  },

  common: {
    projectPath: "项目路径",
    chooseFolder: "选择文件夹",
    newSession: "新建会话",
    newSessionTitle: "新建会话",
  },
};

export default zhCN;
