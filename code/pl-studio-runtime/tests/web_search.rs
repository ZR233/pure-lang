...
use pl_model::config::{
    ...
    ProviderId, ProviderModelCatalogConfig, ReasoningEffort,
};
...
use pl_studio_runtime::search::{
    ToolVisibilityConstraint, WebSearchAvailability, WebSearchBackendKind, WebSearchPath,
    plan_web_search, plan_web_searches,
};
...
fn openai_provider_with_secret() -> ProviderConfig {
    let mut provider = pl_model::config::builtin_provider_catalog()
        .presets
        .into_iter()
...
fn planner_distinguishes_disabled_missing_credential_and_model_support() {
    ...
    let disabled = WebSearchConfig {
        mode: pl_protocol::search::WebSearchMode::Disabled,
