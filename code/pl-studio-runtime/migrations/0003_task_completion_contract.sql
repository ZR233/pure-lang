...
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_outcomes_run_agent_attempt
    ON agent_outcomes(task_run_id, agent_id, attempt);

CREATE INDEX IF NOT EXISTS idx_agent_outcomes_run_status
    ON agent_outcomes(task_run_id, status, updated_at DESC, id DESC);

CREATE UNIQUE INDEX IF NOT EXISTS idx_branch_leases_common_branch
    ON branch_leases(git_common_dir, branch);

CREATE INDEX IF NOT EXISTS idx_merge_records_run_updated
    ON merge_records(task_run_id, updated_at DESC, id DESC);

CREATE UNIQUE INDEX IF NOT EXISTS idx_review_rounds_run_round
    ON review_rounds(task_run_id, round);

CREATE INDEX IF NOT EXISTS idx_task_runs_phase_updated
