...
use pl_tool::environment::{ExecutionEnvironment, ExecutionTransport, ShellDialect};
use pl_tool::workspace::WorkspaceInstructions;
use pl_tool::workspace::load_workspace_instruction_documents;
...
impl InstructionAssembler {
    ...
    fn assemble_inner(
        ...
        if let Some(skills) = request.skills.filter(|skills| skills.enabled) {
            let prompt = request
                .skill_catalog
                .map(pl_tool::skill::build_skills_prompt_from_catalog)
                .map(Some)
                .map(Ok)
                .unwrap_or_else(|| {
                    pl_tool::skill::build_skills_prompt(request.workspace_root, skills, None)
                });
            match prompt {
                Ok(Some(skills)) => {
                    snapshot.push_developer(
        ...
        if let (Some(catalog), Some(suggestions)) =
            ...
            && let Some(prompt) = pl_tool::skill::build_skill_suggestions_from_catalog(
                catalog,
                suggestions.query,
...
mod tests {
    ...
    use pl_tool::environment::{
        ExecutionEnvironment, ExecutionOs, ExecutionTransport, ShellDialect,
    };
    ...
    fn instruction_skill(
        ...
        pl_tool::skill::SkillMetadata {
            ...
            category: None,
            platforms: Vec::new(),
            source: pl_tool::skill::SkillSourceKind::Project,
            path: path.clone(),
            provider_id: pl_tool::skill::SkillProviderId::new("test").unwrap(),
            invocation: pl_tool::skill::SkillInvocationPolicy::default(),
            resource_base: pl_tool::skill::SkillResourceBase::Directory { path },
        }
    }
    ...
    fn uses_preloaded_workspace_documents_for_non_local_workspace() {
        ...
        let documents = pl_tool::workspace::WorkspaceInstructions {
            documents: vec![pl_tool::workspace::WorkspaceInstructionDocument {
    ...
    fn disabled_skills_do_not_inject_a_frozen_catalog() {
        ...
        fs::create_dir_all(&dir).unwrap();
        let skills = pl_tool::skill::SkillsConfig {
            enabled: false,
            ..pl_tool::skill::SkillsConfig::default()
        };
        let catalog = pl_tool::skill::SkillCatalog {
    ...
    fn skill_suggestions_are_turn_overlays_and_exclude_loaded_names() {
        ...
        fs::create_dir_all(&dir).unwrap();
        let skills = pl_tool::skill::SkillsConfig::default();
        let catalog = pl_tool::skill::SkillCatalog {
            project_dir: dir.join(".agents/skills"),
            skills: vec![
                instruction_skill(
