...
mod tests {
    use super::*;
    use pl_core::{context::OpaquePayload, thread::ToolOutcome, tool::ToolOutput};
    ...
    fn reference_with(
        ...
        ResourceReference::new(
            format!("pl.studio.resource:{}", id_seed.to_string().repeat(64)),
    ...
    fn reference(seed: char, media_type: &str, byte_len: u64) -> ResourceReference {
        reference_with(seed, seed, media_type, byte_len)
    }

    fn receipt(version: u32, original: &ResourceReference) -> OpaquePayload {
        OpaquePayload::new(
            "pl.tool.image",
            version,
    ...
    fn delivery(call_id: &str, payload: OpaquePayload, media: Vec<ContextContent>) -> ToolDelivery {
        ToolDelivery {
            target: Default::default(),
            call_id: call_id.into(),
            tool_id: "view_image".into(),
            delivered_context: media.clone(),
            output: ToolOutput::new(payload, media),
            outcome: ToolOutcome::Succeeded,
        }
    }

    fn image(reference: ResourceReference) -> ContextContent {
        pl_model::runtime::attachment_content(reference, AttachmentModality::Image).unwrap()
    }

    #[test]
    fn a_receipt_describing_more_than_one_attachment_is_rejected() {
        let media = vec![
            image(reference('a', "image/jpeg", 12)),
            image(reference('c', "image/jpeg", 12)),
        ];
        let delivery = delivery("call", receipt(1, &reference('b', "image/png", 40)), media);
        assert!(delivery_attachments(&delivery).is_err());
    }

    #[test]
    fn a_receipt_describing_a_non_image_attachment_is_rejected() {
        let media = vec![
            pl_model::runtime::attachment_content(
                reference('a', "application/pdf", 12),
    ...
    #[test]
    fn an_unknown_receipt_version_is_rejected_instead_of_dropped() {
        let media = vec![image(reference('a', "image/jpeg", 12))];
        let delivery = delivery("call", receipt(2, &reference('b', "image/png", 40)), media);
        assert!(delivery_attachments(&delivery).is_err());
    }

    #[test]
    fn identical_references_may_recur_across_calls_but_conflicts_are_rejected() {
        let id = reference('a', "image/jpeg", 12).id().to_owned();
        let first = delivery(
            "call-a",
            OpaquePayload::new("pl.tool.mcp-resources", 1, "{}").unwrap(),
            vec![image(reference('a', "image/jpeg", 12))],
        );
        let same = delivery(
            "call-b",
            OpaquePayload::new("pl.tool.mcp-resources", 1, "{}").unwrap(),
        ...
        assert_eq!(
            persisted_reference(&[first.clone(), same.clone()], &id)
                .unwrap()
                .unwrap(),
            reference('a', "image/jpeg", 12)
        );

        // A later delivery reusing the id with different bytes or modality contradicts the
        // persisted fact and must be rejected rather than resolved to the first match.
        let conflicting_bytes = delivery(
            "call-c",
            OpaquePayload::new("pl.tool.mcp-resources", 1, "{}").unwrap(),
            vec![image(reference_with('a', '0', "image/jpeg", 12))],
        );
        assert!(
            persisted_reference(&[first.clone(), same.clone(), conflicting_bytes], &id).is_err()
        );

        let conflicting_modality = delivery(
            "call-d",
            OpaquePayload::new("pl.tool.mcp-resources", 1, "{}").unwrap(),
            vec![
                pl_model::runtime::attachment_content(
        ...
        assert!(persisted_reference(&[first, same, conflicting_modality], &id).is_err());
    }
}
