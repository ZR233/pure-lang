...
mod tests {
    ...
    fn entry(id: &str, updated_at: i64) -> CatalogEntry {
        CatalogEntry {
            id: id.to_string(),
    ...
    fn temp_path(name: &str) -> PathBuf {
        tempfile::Builder::new()
            .prefix(&format!("pl-studio-catalog-{name}-"))
            .tempdir()
            .unwrap()
            .keep()
            .join("catalog.toml")
    ...
    #[tokio::test]
    async fn missing_catalog_is_an_empty_revision_one_document() {
        let store = CatalogStore::load(temp_path("fresh")).await.unwrap();
        assert_eq!(store.revision(), 1);
        // 启动零驻留：全新安装只暴露带 revision 1 的空目录摘要，不加载任何会话条目。
        assert!(store.entries().is_empty());
    }

    #[tokio::test]
    async fn upsert_round_trips_through_atomic_toml() {
        let path = temp_path("roundtrip");
        let store = CatalogStore::load(path.clone()).await.unwrap();
        store.upsert(entry("a", 10)).await.unwrap();
        assert_eq!(store.revision(), 2);
        store.upsert(entry("a", 10)).await.unwrap();
        assert_eq!(store.revision(), 2, "identical content is a no-op");
        store.upsert(entry("b", 20)).await.unwrap();
        assert_eq!(store.revision(), 3);

        let reloaded = CatalogStore::load(path).await.unwrap();
        assert_eq!(reloaded.revision(), 3);
        assert_eq!(reloaded.entries().len(), 2);
    }

    #[tokio::test]
    async fn page_walks_keyset_and_skips_state_reads() {
        let store = CatalogStore::load(temp_path("page")).await.unwrap();
        for (id, updated_at) in [("a", 10), ("b", 20), ("c", 30)] {
            store.upsert(entry(id, updated_at)).await.unwrap();
        }
        let no_projects = std::collections::HashSet::new();
        let query = ThreadDirectoryQuery::default();
        let first = store.page(&query, &no_projects, None, 2);
        assert_eq!(
            first.iter().map(|t| t.id.as_str()).collect::<Vec<_>>(),
            ["c", "b"]
        );
        let cursor = (
            first.last().unwrap().updated_at,
            first.last().unwrap().id.clone(),
        );
        let second = store.page(&query, &no_projects, Some(&cursor), 2);
        assert_eq!(
    ...
    #[tokio::test]
    async fn search_and_attention_filters_use_only_the_summary() {
        let store = CatalogStore::load(temp_path("search")).await.unwrap();
        let mut waiting = entry("w", 5);
        waiting.title = "需要确认".to_string();
        waiting.status = ThreadStatus::WaitingInteraction;
        store.upsert(waiting).await.unwrap();
        store.upsert(entry("plain", 6)).await.unwrap();

        let no_projects = std::collections::HashSet::new();
        let search = ThreadDirectoryQuery {
            search: Some("需要确认".to_string()),
            ..Default::default()
        };
        assert_eq!(
            store.page(&search, &no_projects, None, 10)[0].id.as_str(),
            "w"
        );
        let attention = ThreadDirectoryQuery {
            filter: ThreadDirectoryFilter::Attention,
            ..Default::default()
        };
        let page = store.page(&attention, &no_projects, None, 10);
        assert_eq!(
    ...
    #[tokio::test]
    async fn archive_and_touch_are_idempotent() {
        let store = CatalogStore::load(temp_path("archive")).await.unwrap();
        store.upsert(entry("a", 10)).await.unwrap();
        store.archive(&["a".to_string()], 50).await.unwrap();
        let revision = store.revision();
        store.archive(&["a".to_string()], 60).await.unwrap();
        assert_eq!(store.revision(), revision, "repeated archive is a no-op");
        let archived = store.get("a").unwrap();
        assert!(archived.archived);
        assert_eq!(archived.updated_at, 50);

        store.touch(&[("a".to_string(), 70)]).await.unwrap();
        assert_eq!(store.get("a").unwrap().updated_at, 70);
        let revision = store.revision();
        store.touch(&[("missing".to_string(), 90)]).await.unwrap();
        assert_eq!(store.revision(), revision, "missing touch is a no-op");
    }

    #[tokio::test]
    async fn corrupt_catalog_fails_closed() {
        ...
        tokio::fs::write(&path, "schema_version = 99\nrevision = 0\n")
            .await
            .unwrap();
        assert!(CatalogStore::load(path).await.is_err());
    }

    #[test]
    fn worktree_entry_requires_a_workspace_address() {
        let mut entry = entry("root", 1);
        entry.workspace_mode = ThreadWorkspaceMode::Worktree;
        entry.workspace_path = String::new();
        ...
        assert!(decode(Path::new("catalog.toml"), &encoded).is_err());
    }

    #[test]
    fn summary_round_trips_to_the_protocol_thread() {
        let entry = entry("root", 7);
        let thread: Thread = entry.to_thread();
        assert_eq!(CatalogEntry::from_thread(&thread), entry);
    }
}
