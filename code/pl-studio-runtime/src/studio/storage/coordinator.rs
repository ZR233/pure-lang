...
mod tests {
    ...
    /// 同一 Thread 的 sink 与实时订阅必须收敛到同一个写者句柄。
    ///
    /// 两个不同时机的"打开"（订阅先登记、sink 后取用）都指向同一连接状态，因此 effect commit
    /// 与 ordinal 预留在同一个有序写者上排队，而不是两条各自独立的 SQLite writer 互抢写锁。
    #[tokio::test]
    async fn one_thread_shares_one_history_writer() {
        let coordinator = ThreadPersistenceCoordinator::default();
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("history.sqlite");

        // 订阅先打开并登记：这就是该 Thread 后续唯一的有序写者。
        let subscriber = HistoryStore::open(&path, "thread").await.unwrap();
        let installed = coordinator.install_shared_history("thread", &subscriber);
        assert!(installed.is_same_handle(&subscriber));

        // sink 随后取用时拿到的是同一句柄，而不是第二条 writer。
        let sink = coordinator.shared_history("thread").expect("shared writer");
        assert!(sink.is_same_handle(&subscriber));

        // 另一个 Thread 不与之共享写者身份。
        assert!(coordinator.shared_history("other").is_none());
    }

    /// 两个生产者同时登记时收敛到一个写者：后到的那个放弃自己刚打开的句柄。
    #[tokio::test]
    async fn racing_producers_converge_on_one_history_writer() {
        let coordinator = ThreadPersistenceCoordinator::default();
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("history.sqlite");
        let sink = HistoryStore::open(&path, "thread").await.unwrap();
        let live = HistoryStore::open(&path, "thread").await.unwrap();

        let first = coordinator.install_shared_history("thread", &sink);
        let second = coordinator.install_shared_history("thread", &live);
        assert!(first.is_same_handle(&sink));
        assert!(
            second.is_same_handle(&sink),
            "第二次登记必须复用既有活跃句柄，而不是各留一条连接"
        );
        assert!(!second.is_same_handle(&live));
    }

    /// 最后一个真实持有者释放后登记项失效：没有永不失效的强引用缓存，重新激活会打开新写者。
    #[tokio::test]
    async fn a_released_thread_history_writer_is_not_cached_forever() {
        let coordinator = ThreadPersistenceCoordinator::default();
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("history.sqlite");
        ...
        // 关闭/淘汰 Thread：所有真实持有者都释放。
        drop(sink);
        drop(installed);
        drop(subscriber);
        assert!(
            coordinator.shared_history("thread").is_none(),
            "没有持有者时不得继续握着这条连接"
        );

        // 重新激活：登记一个新的写者身份，而不是复活已经释放的连接。
        let reactivated = HistoryStore::open(&path, "thread").await.unwrap();
        let installed = coordinator.install_shared_history("thread", &reactivated);
        assert!(installed.is_same_handle(&reactivated));
        assert!(
            coordinator
