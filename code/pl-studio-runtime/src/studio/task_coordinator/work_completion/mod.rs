...
mod tests {
    ...
    #[test]
    fn completion_identity_keeps_runtime_parent_id_separate_from_logical_owner_path() {
        let root_session_id = "session-root";
        let root_agent_id = crate::studio::agent_host::root_agent_id(root_session_id);
        let subagent = SubagentContext {
            id: "agent-executor".to_string(),
            parent_id: Some(root_agent_id.to_string()),
            agent_path: Some("/root/agent-executor".to_string()),
            role: "executor".to_string(),
            task: "implement".to_string(),
            depth: 1,
        };

        assert!(is_direct_task_child(&subagent, root_session_id));

        let mut legacy_context = subagent;
        legacy_context.parent_id = Some("/root".to_string());
        assert!(!is_direct_task_child(&legacy_context, root_session_id));
    }

    #[test]
    fn report_completion_input_matches_camel_case_tool_schema() {
        let delivery = serde_json::from_value::<ReportCompletionInput>(serde_json::json!({
            "result": {
                "kind": "delivery",
                "headCommit": "abcdef1",
        ...
        assert!(matches!(
            ...
            CompletionResultInput::Delivery {
                head_commit,
                verification_summary
            } if head_commit == "abcdef1" && verification_summary == "verified"
        ));

        let no_delivery = serde_json::from_value::<ReportCompletionInput>(serde_json::json!({
            "result": {
                "kind": "noDelivery",
                "verificationSummary": "nothing to change"
            }
        }))
        .expect("noDelivery input");
        assert!(matches!(
            no_delivery.result,
            CompletionResultInput::NoDelivery {
                verification_summary
            } if verification_summary == "nothing to change"
        ));
    }
}
