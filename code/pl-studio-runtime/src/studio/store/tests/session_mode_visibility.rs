...
async fn legacy_mode_sessions_remain_stored_but_are_not_loadable() {
    ...
    let legacy_auto = store
        .create_session(&project.id, "Legacy auto", StudioMode::Simple)
        .await
        .unwrap();
    let legacy_plan = store
        .create_session(&project.id, "Legacy plan", StudioMode::Task)
        .await
        .unwrap();
    let current = store
        .create_session(&project.id, "Current", StudioMode::Simple)
        .await
        .unwrap();
