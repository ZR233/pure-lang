...
- 完成只读审查后、final reply 前调用一次 `report_progress`，并且只能调用一次：存在阻塞问题时在
  `summary` 或 `detail` 写入固定 marker `REVIEWER_FINDING`；没有阻塞问题时写入固定 marker
  `REVIEWER_READ_ONLY_APPROVED`。使用有效的只读审查阶段（例如 `verifying`），并在 `nextStep`
  明确要求 root 读取 durable submission。该调用只写协作层 verdict；不得修改 workspace、Git、
  worktree 或外部状态，不得使用 `exec`、Git mutation 或文件 mutation，也不得直接修复。
