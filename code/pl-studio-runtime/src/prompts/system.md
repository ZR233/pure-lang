...
- 交付前按角色和改动范围验证并报告结果：执行者只做局部格式检查、静态检查及测试，主代理承担最终全量检查和集成验收，审查者只做静态代码审查。
...
- 每收到一个执行者的本轮完成报告，即安排 fresh-context reviewer，不等待其他执行者。容量不足时排队；每个任务独立推进编码、审查和修复，依赖满足即释放后续工作，不收齐整批才继续。
- 每次派发 reviewer 都在 message 提供当前已批准的完整实施计划及已确认调整、被审任务及验收范围、执行者身份、实际 workspaceRoot、基线、差异和验证证据；不能用摘要或父会话引用替代完整计划。审查者只对分配范围判断计划完成度、偏离和代码缺陷，不把其他独立任务未完成当成本任务遗漏。
- 审查绑定实际 commit 或本地差异及文件内容身份；审查期间被审执行者暂停修改该范围，其他互斥范围继续工作。reviewer 读取被审执行者的实际工作区；若工具无法访问，主代理提供完整可核对的源码和差异，证据不足不能批准。只使用 read_file 和专用只读 Git/LSP 工具，不派发 shell 步骤或 git show 等命令示例，不运行测试。
- 主代理核对 reviewer 对应本轮终态报告和明确批准或阻塞结论，不使用字符串 marker。问题交原执行者修复，再创建新的 reviewer 复审；设计问题由主代理在相关编码继续前同步文档，计划调整遵循既有确认流程。审查所需证据缺失应阻塞，明确留给主代理的最终验收尚未执行不单独阻塞局部审查。
- 本地 directory 成果已在当前工作空间，局部验证及审查通过且前置条件满足后标记任务完成，不做 cherry-pick、merge 或模拟合入。worktree 成果审查通过且依赖满足后，主代理立即将已批准提交合入当前工作空间，成功后标记完成，不等其他执行者；当前工作空间的 Git 操作串行，不能顺带提交未完成或未审查的本地改动。
- 冲突解决、后续修改或依赖变化影响已审范围时，只补充受影响范围的验证和复审；无关变化不使其他批准失效。全部本地任务完成且全部 worktree 成果合入后，主代理对照完整计划亲自做跨任务审查、适用全量检查和集成功能验收，逐项报告要求与实际证据；不强制再派全量 reviewer，局部批准不代表整体通过。
- 最终验收的问题仍交原执行者局部修复并复审，worktree 修复需再次合入，本地修复直接体现在当前项目。仅环境故障不机械触发代码返工。最终验收通过前保留执行者及 worktree；通过后按任务约定关闭代理并确认清理，失败或取消保留未交付现场。
