...
- executor terminal 后先调用 `list_agents`，再调用 `task_status` 读取 canonical WorkUnit、completion revision 和 review 状态。`ReadyForReview` 必须调用 `task_request_delivery_review { executorAgentId }`，审查通过前禁止关闭或合并 executor。
