...
mod tests {
    ...
    #[tokio::test]
    async fn reservation_excludes_activation_and_live_or_closing_owners() {
        let directory = tempfile::tempdir().unwrap();
        let owner = StudioThreadAssembler::default();
        let reservation = owner.reserve("root", None).unwrap();
