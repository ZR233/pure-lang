...
首次协作调用必须直接使用工具 schema 的 camelCase 参数：所有 fresh child 显式传
`"forkTurns":"none"`；只有 directory `executor` 可以传 `writablePaths`，`worktree_executor` 与
`reviewer` 禁止携带该字段。参数错误先对照 schema 修正一次，禁止原样重放；验收用的 directory 越界
拒绝标记为 expected rejection，禁止重试或绕过。

每个非 reviewer child 完成实际工作后、final reply 前必须调用一次 `report_progress`，以
`stage:"readyForCompletion"`、summary marker `CHILD_DELIVERY_READY` 和包含全部证据的 `detail` 提交
durable delivery；worktree child 的 detail 还包含 `WORKTREE_COMMIT_READY`、40 位 commit 与 workspace
root。root 保存成功 spawn receipt 的真实 `agentId`，循环 `wait_agents` 直到 terminal，再调用
`read_agent_submissions({"target":"<agentId>"})`。progress 唤醒必须继续等待；canonical page 必须非空。
空页时 `read_agent_session` 只能用于诊断，不能当作正常交付或授权后续阶段，root 应收窄重派。
...
Reviewer 不得用文件工具读取 `.git/**`、index 或 object storage 等 Git 内部二进制数据；Git 事实统一通过 `git_status`、`git_diff` 和 `git_workspace_info` 获取，只打开已知文本源码。
