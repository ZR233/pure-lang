...
use crate::default_models::{default_models, openai_default_model_slugs};
    ...
    pub(crate) fn new(info: ProviderInfo, configured_models: Vec<ModelInfo>) -> Result<Self> {
        Ok(Self {
            inner: OpenAiTransportProvider::new(
                info,
                bundled_models(openai_default_model_slugs()),
                configured_models,
...
pub(crate) fn bundled_models(slugs: &[&str]) -> Vec<ModelInfo> {
    default_models()
        .into_iter()
