...
mod tests {
    ...
    fn child_process_verifies_filter_priority_error_mirror_and_flush() -> Result<()> {
        ...
        let default_logs = read_main_logs(&default_root.join("anywork").join("logs"))?;
        let default_errors = read_logs(&default_root.join("anywork").join("logs"), "error-")?;
        ...
        let rust_log_contents = read_main_logs(&rust_log_root.join("anywork").join("logs"))?;
        ...
        let cli_logs = read_main_logs(&cli_root.join("anywork").join("logs"))?;
    ...
    fn child_process_persists_panic_to_crash_and_error_logs() -> Result<()> {
        ...
        let diagnostics_root = root.join("anywork");
    ...
    fn child_root(label: &str) -> Result<PathBuf> {
        ...
        let root = std::env::temp_dir().join(format!(
            "anywork-diagnostics-{label}-{}-{nonce}",
            std::process::id()
        ));
