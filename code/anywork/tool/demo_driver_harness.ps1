...
$appDir = Join-Path $repoRoot 'code\anywork'
if ([string]::IsNullOrWhiteSpace($ArtifactsDirectory)) {
    $ArtifactsDirectory = Join-Path `
        ([System.IO.Path]::GetTempPath()) `
        "anywork-demo-driver-$([Guid]::NewGuid().ToString('N'))"
}
...
try {
    $guiProcess = Start-LoggedProcess `
        ...
        -Environment @{
            'ANYWORK_HOME' = Join-Path $ArtifactsDirectory 'studio-home'
            'ANYWORK_NATIVE_LIFECYCLE_LOG' = Join-Path $ArtifactsDirectory 'native-window.log'
        }
