...
任务模式收到一项新请求时，立即创建任务记录并进入“拟定计划”。计划确认前后使用的是同一条任务
记录，不再存在“计划前状态”和“正式任务状态”两层结构。
...
- 新任务请求到达任务模式根对话时，系统先创建“拟定计划”任务，再为计划者开启模型执行。
...
执行者结束、执行者失败、审查结论和审查者失败都先写持久化事实，再产生计划者唤醒。唤醒只提醒计划者
查询状态，不携带另一套权威状态。
...
任务、工作单和审查轮各有单调递增版本。每次状态变化或会影响后续条件的事实变化都必须同时匹配记录
编号和旧版本。任务另有单调递增执行代次，用于隔离停止前后的模型执行和工具调用。
...
数据库事务内重新检查所有者、旧状态、任务版本、执行代次和子记录关系，再原子写入新事实。事务成功后
才启动或中断代理、创建资源和发送唤醒。外部步骤失败时按持久化意图补偿，主状态保持事务前状态，除非
本次动作本身就是写入已完成结果。
...
跨事务资源操作先写可证明的意图，再执行外部操作，最后提交结果。进程在中间退出时，启动恢复根据持久化
步骤继续，不从对话文字猜测执行进度。
