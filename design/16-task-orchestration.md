...
桌面界面可以把“非终态任务当前没有根计划者、执行者或审查者执行，也没有等待用户答复”展示为
“执行已暂停，发送消息可继续”。这是由 canonical Thread 执行状态和 Task 子记录共同派生的展示事实；
界面仍须同时保留任务原本的六状态，不得把“暂停”写回 `TaskRunState` 或伪装成第七种主状态。
...
查询的任务 `state` 不能因为当前没有活动执行就变成“暂停”，也不能因为资源问题变成“阻塞”。
独立的执行活动和桌面展示可以如实表达当前没有模型执行、需要用户发送消息继续。默认结果必须有界；
...
模型调用 `task_spawn_executor` 时，范围和验证契约分别以 `scope`、`verification` 的结构化 JSON object
传递；不能把这两个对象预序列化成 JSON 字符串。模型可见 schema 必须直接展开这两个 object 的字段，
不能要求兼容 Provider 额外解析属性级 `$ref` 才能知道其对象形状。
...
审查提示和提交门禁以工作目录中的 design Markdown 索引为准。索引非空时，审查者必须先定位并实际
读取至少一个相关 `design/**` 文档，且提交真实章节引用；索引为空表示项目没有 design 文档，此时允许
`designReferences` 为空并跳过 design 读取与引用门禁。文件定位、逐文件覆盖和其他审查合同不受影响。
...
单执行者免审复用的是获准交付、通过的交付审查和合并记录之间的身份与版本关系；整合方法可以产生新的
结果提交，因此不要求合并记录的结果提交与交付提交具有相同提交 ID。
...
项目级移除在关闭 agent tree 后，必须直接把该项目所有剩余 pending Interaction 持久化为 cancelled
并释放进程内 waiter，再执行资源清理和目录移除。已关闭或非驻留 Thread 不得因为缺少 ThreadActor 或
内存目录项阻断清理；该直接持久化边界只适用于即将整体移除的项目，不替代活动 Thread 的 canonical
notification 提交链。
