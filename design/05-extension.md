...
Subagent 工具通过 `spawn_agent`、`wait_agent`、`list_agents`、`send_message`、`followup_task` 和 `close_agent` 暴露。`spawn_agent.forkTurns` 可取 `none`、`all` 或正整数字符串；默认 `none`，表示子代理只接收初始任务消息。显式请求 `all` 或 `N` 时，运行时从父会话构造过滤后的历史快照：保留 system/user 消息和 assistant 最终文本，丢弃工具调用、工具结果、reasoning 内容和运行时提示，避免把大输出复制给子代理。`wait_agent` 和 `list_agents` 默认返回紧凑摘要；只有显式传入 `includeDetails: true` 才返回完整 agent record，用于调试或诊断。Studio 的 agent 展示继续以事件流和 store snapshot 为准，不依赖模型上下文中的详细 tool result。
