...
anywork 使用新的固定 Windows AppId，默认安装到本地程序目录，与旧版 Pure Studio 独立
安装和卸载；新快捷方式、WER 配置和卸载清理仅管理 anywork。继续使用既有版本序列、签名
信任根与更新清单地址，但资产名称改为 anywork 前缀；旧版的文件名校验会拒绝新版清单，
用户必须手动下载安装 anywork，且不会带入旧配置、聊天记录或 API key。
