...
沿滚动方向距边缘 1.5 个视口时预取；不足一屏自动补齐，直到填满、到端或失败。
向旧加载不要求先脱离底部：首窗为空或条目经隐藏、分组后不足一屏时也必须能继续读取历史。
自动补齐沿同一个阅读方向进行，到端后不反向补页，避免有界窗口在两端之间往返。
没有滚动范围时，滚轮和拖动仍可表达翻页方向；显式跳到最新后保持最新窗口，直到再次上翻。
向旧加载淘汰远端新条目，向新加载淘汰远端旧条目；可见条目与阅读锚点优先保留。失败保留正文，
在对应边缘提供显式重试，另一方向不受影响；超过 150ms 才显示不占正文高度的加载提示。
分页或聚焦改变分组边界时，仍有共同条目的推理组和工具组保留展开状态，避免阅读过程中
因分组首条变化而收起正文；离开窗口的分组状态仍随窗口释放。

用户上翻即暂停自动跟随，即使仍在底部阈值内也保持手动阅读；新输出、内容补齐、图片展开、
分页和窗口缩放不能恢复跟随。只有用户向下滚回最新端或显式跳到最新，才同时恢复界面跟随
与 ChatView 的 Latest 聚焦；锚点校正不得取消进行中的拖动。
