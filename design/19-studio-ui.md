...
只属于包含该 Turn 末条的覆盖范围。Turn 历史消费者也必须使用有界条目页，不保留整 Turn
全量正文的旁路。首次选择会话只读取最新一页，不激活执行 owner；HTTP 与 FRB 使用同一
窗口、水位和游标语义，禁止在 bridge 全量转换后截断冒充分页。
页面及缓存同时受字节预算约束。超大条目先显示有界预览，按需展开并分段读取完整正文，
引用包含内容版本与摘要；切换会话或窗口后旧分段响应不得污染新窗口，不自动下载全部正文。
