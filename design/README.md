...
| 17 | [Studio 存储与诊断](./17-studio-storage.md) | 双库、checkpoint 与分页、恢复与协调重置、诊断、LRU |
...
- 只描述设计目的、原理、接口形状、逻辑与约定；不出现代码块、方法签名、内部模块路径、
  源文件路径或行号。crate 名、协议/工具/事件/wire 字段名、配置键与库表名属于接口形状，
  可以保留。
...
- 命名口径：系统名 Pure-Lang；桌面应用 anywork（中文名"糊来帮"）；Rust crate 统一
  `pl-` 前缀。
