...
状态：已采纳的重构设计。基于 Flutter UI 与 `design/11-studio-ui.md` 的暹罗色系，替换全局新会话入口及项目、会话平铺分区。
...
3. 远程：显示已有 SSH 配置列表、搜索及“新建远程配置”。已有配置展示名称、用户、主机、端口及带时间的上次连接结果；未经当前验证不标为在线。无配置时直接展示空态及新建入口。
4. 选择已有配置，点击“连接并选择目录”；显示连接进度，失败可重试、编辑配置或返回选择其他连接。
5. 新建配置：在原弹窗内进入表单，收集名称、主机、端口（默认 22）、用户名、认证方式；SSH Agent/密钥模式显示可选私钥，密码模式显示隐藏密码。依现有认证契约提供字段，不发明额外认证协议。字段错误就地提示。
6. “保存并连接”明确保存配置并尝试连接。保存失败保留表单；保存成功但连接失败明确显示“配置已保存，连接失败”，重试复用同一配置。返回不会重复创建。尚未保存的草稿在步骤间保留，取消整个流程后丢弃。
...
Flutter 侧栏与向导位于 `code/anywork/lib/src/features/shell/`；SSH 配置及目录步骤复用设置组件。目录查询、归档恢复、项目名称及 UI preferences 由 StudioRuntime 提供 canonical 状态，经 FRB 与 HTTP 使用同一契约。归档保留的热事实覆盖未落盘冷行，耐久后可从热集合淘汰；异步工具刷新不能为已退役的会话实例重新发布恢复错误。

项目内第一页为 8 个普通会话，并附带配置中的置顶会话；后续每页 20 个，按身份去重。归档列表按当前打开项目组织；关闭的项目需要重新打开原目录后恢复其历史。宽度和两类置顶通过现有 Settings CAS 持久化，每类置顶最多 64 项。

Linux 原生截图及向导交互验收：`python3 code/anywork/tool/sidebar_native_harness.py target/sidebar-native`。它在隔离 X11 会话中通过 Flutter Driver 操作，保存宽屏、添加项目、远程配置、目录和窄屏截图，并验证所属进程回收；不访问真实 SSH 主机。真实运行时的目录／归档／恢复行为由 Rust 集成路径验证，实际用户主机连接依现有 SSH 实现。
