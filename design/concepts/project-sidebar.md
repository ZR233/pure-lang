...
- 归档是破坏性动作：确认后 runtime 一并清理该会话树在 worktree 模式下的工作树（含其中未提交、未整合的内容），确认文案必须说明该后果。清理失败不静默：回落保留现场并发布诊断。恢复会话时在原路径重建工作树，使会话地址保持稳定。
...
原生截图与向导交互使用统一的 GUI 人工验收入口，在隔离会话中观察并核对进程回收，不访问真实 SSH 主机；已移除 Studio 自动集成测试，目录、归档与恢复仍须在人工验收中单独检查。
