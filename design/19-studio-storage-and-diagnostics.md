...
完整 workflow state 最大 256 KiB；definition 与尾部历史在进入 repository 前已由 core 验证。
...
Mode Skill 仍由 Skill Provider 提供，不复制到数据库。run 内保存完整 `ModeInstructionSnapshot`，所以
来源 Skill 修改或删除不会改变 active run。
