...
`AgentRuntimeHandle::activate` 按 ID 串行恢复会话；驻留和冷历史查询共用 core 接口。
`ThreadRepository` 的 flush 固定当前待提交水位，shutdown 在所有 actor 停止后排空并关闭 writer。
保存失败或 worker panic 必须返回错误并保留待保存事实；不能以 stopped 标志替代成功。
产品数据库仅记录是否曾注册会话的关联 receipt，实际 revision 只属于 core。
启动对账区分未激活目录、缺失会话和缺失产品关联；后两者报告错误并保留资源，不创建空会话掩盖历史丢失。
