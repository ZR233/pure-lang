...
`spawn_agent`、`send_input`、`wait_agent`、`list_agents`、`close_agent` 和 `resume_agent` 的模型可见输出必须通过 pl-core typed helper 或 output type 构造，宿主 adapter 只填入产品侧 agent identity、状态、路径和诊断快照，不手写共享字段形状。`wait_agent` 只回传 `{ message, timedOut }`，避免把完整 agent snapshot 反复写入模型上下文。单目标 wait snapshot、turn start snapshot 与 wait outcome 都由 pl-core constructor 承载字段形状；多目标等待场景中，completed/pending 分区如何投影为 wait loop snapshot，以及 `message` 内部的 `{ completed, pending, timedOut }` 紧凑 JSON，都必须由 pl-core typed helper 生成；宿主只提供产品侧 agent 快照，不重复拼模型可见输出字段或分组等待语义。需要状态明细时调用 `list_agents` 获取当前 compact snapshot；Studio 展示依赖 `AgentChanged` latest snapshot 和 `SubAgentActivity` / `TodoListUpdated` append-only timeline。`spawn_agent.forkTurns` 的历史继承只复制过滤后的父会话消息，不复制工具结果、工具调用 metadata、reasoning 内容或运行时调度提示。
