...
## ToolManager、AgentToolSet 与 step 代际

`pl-core::ToolManager` 是工具所有权、注册、快照、调用与审计的唯一入口。builtin、host、MCP、
LSP、Task、协作工具与 provider hosted tool 一律先构造成同一个 `Tool`，再注册进 manager；
`TurnEngine`、provider adapter 和产品 runtime 都不得保存第二份 name-to-handler 映射或直接调用
`Tool::execute`。

provider 无关的 wire 定义是 `pl_protocol::ToolSpec`。`Tool::spec` 返回该不可变定义；可信
effect、并行/锁、预算、programmatic eligibility 与执行缓存策略是 `Tool` 的独立 typed 属性，
不污染 provider wire。`ToolExecution` 穷尽区分本地 executor 与 provider hosted executor。
typed function helper 只负责由 Rust 输入类型生成严格 schema 并构造普通 `Tool`，不形成独立的
definition、registry 或 dispatch 路径。

`ToolManager` 拥有一个 global 工具集，并为每个 agent 创建持久 `AgentToolSet`。agent 创建时
显式选择是否继承 global；同一 scope 内的空名或重名使整批注册失败，agent-local 同名工具覆盖
继承的 global 工具。角色、Task phase 与产品能力通过“向该 agent 注册哪些工具”表达，不在模型
请求组装后再按名字删除，也不按来源或前缀提升权限。

动态来源通过 `AgentToolSet::replace` / `install` 原子发布单组完整下一代；需要同时更新多个来源时
使用 `replace_batch` / `install_batch`，全部 group 在同一 scope 写锁内校验并一次提交。构建、
冲突或刷新失败时旧集合原样保留。global scope 使用 `ToolManager::replace_global` /
`replace_global_batch`。注册返回的 RAII guard 只撤销自己对应的 generation，旧 guard 不得删除
后继 generation。group 身份只服务生命周期、回滚和诊断，不参与模型可见名称、prompt wire 或
权限判断。

每次模型 step 在 provider 请求之前开放一次 `before_model_step` 刷新窗口。刷新完成后，
`AgentToolSet::freeze` 产生不可变 `ToolPlan`，同时保存 canonical `ToolSpec` 列表、精确的
name-to-executor 映射、wire fingerprint 与每项 execution generation。
同一次 provider 调用、其 transport retry、返回工具调用及工具执行全部使用同一个 plan；注册变化
不终止或重写在途请求，只从下一次 model step 生效。新 plan 的 schema 变化会让 Responses
WebSocket continuation 自然退回 full request，但不重建 `ModelSession`。

`ToolPlan` 的 wire fingerprint 只取决于实际发送的 canonical specs；executor generation 只进入
dispatch 与工具结果缓存 key。因而相同 schema 的 handler replacement 不破坏 provider prefix，
又不会错误复用旧 handler 的本地执行结果。已冻结 plan 持有 executor 的共享 owner，旧 generation
只有在最后一个 plan 和在途调用释放后才能关闭。

普通工具使用平铺 model-visible name；同 scope 重名 fail-loud，agent scope 按上述规则 shadow
global。MCP 是唯一强制命名空间的来源：公开名为
`mcp__<本地 server id>__<raw name>`，有损归一化或截断时追加稳定哈希，dispatch 保存 raw identity
且绝不反解析公开名。

## 破坏性 API 迁移

本次重构不提供兼容层。旧 `ToolRegistry` / `ToolEntry` / `ToolSourceId` /
`TurnToolLease` / `ToolSetBuilder` / `ToolVisibilitySet` / `FunctionToolDefinition` /
`RegisteredTool` 与延迟 Tool Search API 全部删除：

- 应用创建唯一 `ToolManager`，为每个 durable agent 创建并保存一个 `AgentToolSet`；global 继承在
  `agent_tool_set` 时显式选择，不存在隐式共享注册表。
- 静态 typed handler 使用 `TypedTool::handler`，运行时 handler 使用 `LocalTool`，开放扩展直接实现
  `Tool`；三者最终都以 `Arc<dyn Tool>` 进入同一个 `install` / `install_batch` 路径。
- 宿主 installer 用稳定 `ToolGroupId` 发布其完整集合。动态刷新必须在 `before_model_step` 中完成，
  并在返回前处理所有失败；跨来源一致性使用 batch API，不能先发布半组再回滚。
- provider adapter 只接收 `ToolPlan::specs()`；本地调用只能用同一 plan 交给
  `ToolManager::execute`，不得保存 handler 或按名称直接调用 `Tool::execute`。
- MCP adapter 保存 `(server_id, raw_name)` identity 并生成 namespaced visible name；LSP、hosted Web
  Search 与 Programmatic Tool Calling 都只是按能力条件安装或卸载的普通 group，不是 manager
  之外的控制面旁路。

使用 `pl-core` 的其他应用可以自行决定工具的构造、依赖捕获和 agent 分组，但不能替换上述
manager/plan 生命周期。工具集合不变时，注册顺序和 execution generation 不得改变 provider wire；
工具集合变化时从首个变化的 request header 自然失效，不派生新的 provider cache key。
...
1. 检查当前模式是否允许该工具。
2. 在本 model step 的冻结 `ToolPlan` 中回查工具 binding。
...
MCP runtime 在连接 generation 激活时把该代全部工具与 resource façade 一次性原子发布到目标
agent 的 MCP group，不在每个 Turn 临时安装或卸载。工具 handler 捕获所在 generation 的调用通道、
server id 和远端 raw tool name，因而与内置工具共享模式门控、权限与
...
配置更新或 `tools/list_changed` 只影响下一次 step plan，不得改变活动 plan 冻结的连接或工具集合；
重新 list、命名或发布失败时继续保留旧 generation。
...
缓存与工具集合只使用一个 wire 事实：`ToolPlan::wire_fingerprint`。它是本 step 实际发送的完整
可见 `ToolSpec` 集合的 canonical 哈希；agent/global revision、group id、注册顺序和 executor
generation 仅作 dispatch/诊断，不参与 wire。全部工具 eager 发送，不存在客户端 Tool Search、
deferred catalog、`NamespaceDescriptor` 或 catalog fingerprint。

工具按模型可见名称和 canonical JSON Schema 确定性排序；同一个 plan 在 provider retry 与工具
执行期间不可替换，也不能为了缓存命中跨 Thread、Workspace 或 agent 工具集复用。工具结果无论
成功、失败或拒绝都只追加到 durable history，不得重排旧输入。工具列表变化形成新的 request
header，并使 Responses WebSocket continuation 自然回退 full request；列表不变时 system、tools
与既有 history 必须保持 byte-identical。
...
路径和权限校验；runtime 不因为 caller 为 program 绕过任何本地策略。
...
每个 model step 冻结 plan 后，运行时把 Turn id、step、实际工具名、wire fingerprint 与 execution
fingerprint 记录为内部诊断 trace；对应 inference 的完成 trace 保存 provider 真实报告的
cache-read/cache-write usage。二者不进入模型上下文，也不创建可见 Item，更不能用本地 fingerprint
伪造缓存命中；旧 `timeline_events` 表已删除。
...
`Tool` 是唯一 authoring trait。它通过 `spec()` 返回不可变 `ToolSpec`，通过 `execution()` 返回
穷尽的 `ToolExecution`。由于 manager 必须持有开放扩展的 `Arc<dyn Tool>`，trait-object 边界使用
显式 `BoxFuture` 擦除本地异步实现；typed `LocalTool` / `TypedTool` 构造器负责把普通 async handler
收敛到该边界。provider hosted tool 由 core adapter 实现同一 trait且 execution target 明确为
hosted，manager 永不走本地 dispatch。全仓不引入 `#[async_trait]`。

## Typed function tool 定义

内建工具和产品静态工具的参数类型、反序列化规则与模型可见 JSON Schema 必须来自同一个
Rust typed definition。输入类型使用 `Deserialize + JsonSchema`，字段名和枚举 wire 值由 Serde
属性声明，模型可见说明由 rustdoc 或 Schemars 属性声明；`TypedTool<Input, Handler>` 只是普通
`Tool` 的便利构造器，统一规范化 schema、拒绝未知顶层字段并反序列化 arguments。它进入相同的
manager、plan 与 dispatch，不保留 `FunctionToolDefinition`、`RegisteredTool` 或第二套公开
authoring API。静态工具不得手写 `properties`、`required` 或 `additionalProperties`，也不得在各
工具内遍历或修补生成后的 JSON Schema。
...
远端 MCP schema、动态 `DynamicTool::new`、运行时 role/target enum，以及 Custom、
Programmatic Tool Calling 和 Hosted Web Search 协议类型。客户端 Tool Search、Namespace 与
deferred catalog 已删除。MCP schema 由 rmcp 提供，
...
MCP tool 结果统一由一个 rmcp `CallToolResult` 转换器生成 `ToolResult`。模型可见字符串按 MCP
...
server。下一 model step 不再获得该 server 的工具，持有旧 plan 的调用仍按固定 generation 收尾。
分页、SSE 与 JSON-RPC/MCP wire 细节全部由 rmcp 处理；PL MCP runtime 只按 connector、worker、
generation、统一 Tool adapter 和结果转换器拆分。
...
Flutter 为 `lsp_query` / `lsp_capabilities` 提供专用图标与带 languageId/查询摘要的参数化标题；
工具名保持原始领域值。客户端 `tool_search` Item 与专用展示已随延迟目录一起删除。
