...
ChatView 测试覆盖新旧 revision 保存确认竞争、缓存/未保存/历史合并无空洞、多个聚焦位置、
慢消费者 Reset、查询失败传播及被淘汰窗口数据的释放。`pl-model` 从单模型运行时与 Thread model
...
重试、checkpoint 发布失败后的 history fence、任务权威读取、最近 100 条窗口及旧会话隔离。
blob 损坏、统计消费者退出、多在途工具完成及关页后的长期保留尚无完整专项注入覆盖，
不能将门禁绿色视为已通过这些场景。
...
当前模拟流同时包含独立条目与持续流式增量；多会话场景未通过之前，整体压力目标仍未完成。
还需分别观察历史条数长、单条正文长、单个 Turn 长、会话数量多；不能以单次流速代替这四项
性能证据。未观察到屏幕更新、卡死、未退出的子进程或未核查截图时，不得判为 GUI 压测通过。
