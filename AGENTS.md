...
- Rust 模块统一使用 `foo/mod.rs` + `foo/child.rs` 目录结构；新模块和已有模块都不保留“文件夹同名 `.rs` 作为模块入口”的结构。
...
以下内容保留通用 Rust 质量准则、示例和检查口径；项目特有约束以前文“工程边界与协议”为准。
...
Rust 支持把模块拆到同名文件或同名目录下的 `mod.rs` 中，但同一个模块不能同时使用 `foo.rs` 和 `foo/mod.rs` 两种文件。Rust Book 也建议模块增长时拆到独立文件，保持 `mod` 声明处的模块树不变。Pure-Lang 统一采用目录模块 + `mod.rs` 作为模块入口。([Rust 文档][23])

选择规则：

- 新模块必须使用 `foo/mod.rs` + `foo/child.rs` 风格，保持目录模块结构一致。
- 已有模块也必须符合 `foo/mod.rs` + `foo/child.rs` 风格；发现 `foo.rs` 入口时，应迁移为目录模块，而不是继续保留同名文件模块入口。
